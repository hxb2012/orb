  ...
  (when-let
      ((result
        (orb-db-query
         (list
          "
SELECT file.file_id, file.keywords, file.tags, file.todo_keywords, file.done_keywords, json_extract(node.properties, '$.COLUMNS')
FROM %s.file AS file
...
ORDER BY level ASC) as tags,
json_each(tags.value) AS t) AS alltags,
    (SELECT json_group_object(key, value) FROM json_each(coalesce(node.properties, '{}')) WHERE value != 'nil') AS props,
    node.properties AS propslitnil,
    (
WITH RECURSIVE
  property(parent_id, level, value) AS NOT MATERIALIZED (
    SELECT node.parent_id, node.level, (SELECT json_group_object(key, value) FROM json_each(coalesce(node.properties, '{}')) WHERE value != 'nil')
    UNION ALL
    SELECT parent.parent_id, parent.level, json_patch((SELECT json_group_object(key, value) FROM json_each(coalesce(parent.properties, '{}')) WHERE value != 'nil'), property.value)
    FROM %s.node AS parent
    JOIN property
    ON parent.node_id = property.parent_id)
SELECT value
...
WITH RECURSIVE
  property(parent_id, level, value) AS NOT MATERIALIZED (
    SELECT node.parent_id, node.level, coalesce(node.properties, '{}')
    UNION ALL
    SELECT parent.parent_id, parent.level, json_patch(coalesce(parent.properties, '{}'), property.value)
    FROM %s.node AS parent
    JOIN property
    ON parent.node_id = property.parent_id)
SELECT value
...
(defun orb-agenda--get-node-scheduled (file condition &optional todo-only)
  (when-let
      ((result
        (orb-agenda--query
         file
  ...
  parent AS NOT MATERIALIZED (
    ...
    ON node.parent_id = parent.node_id)"
                   ...
                   (apply
                    ...
                    (mapcar
                     (lambda (prop)
                       (list ", json_extract(node.properties, '$.' || ?)" prop))
                     props))
