...
(defvar orb-db-filewatch-hook nil)
...
(defun orb-db-filewatch--inotify-callback (event)
  (let* ((desc (car event))
    ...
    (pcase-exhaustive actions
      ('(modify)
       (run-hook-with-args 'orb-db-filewatch-hook 'modified filename))
      ('(isdir create)
  ...
  (if desc
      (let ((pair (assoc desc orb-db-filewatch--desc-alist)))
        (if (not (orb-file-p filename))
            (orb-db-filewatch--remove-file desc)
          (run-hook-with-args 'orb-db-filewatch-hook 'renamed (cdr pair) filename)
  ...
  (if desc
      (let ((dirname (file-name-as-directory filename))
        ...
        (if (not (orb-file-p dirname))
            (orb-db-filewatch--remove-directory desc t)
          (run-hook-with-args 'orb-db-filewatch-hook 'renamed srcname dirname)
...
(defun orb-db-filewatch--remove-file (desc)
  (when-let ((filename (cdr (assoc desc orb-db-filewatch--desc-alist))))
    (run-hook-with-args 'orb-db-filewatch-hook 'deleted filename)
    (setq orb-db-filewatch--desc-alist
...
(define-minor-mode orb-db-filewatch-mode
  ...
  (cond
   (orb-db-filewatch-mode
    ...
    (add-hook 'orb-db-filewatch-hook 'orb-db-filewatch--update-db)
    (orb-db-sync-files
     (seq-remove
      'directory-name-p
   ...
   (t
    ...
    (remove-hook 'orb-db-filewatch-hook 'orb-db-filewatch--update-db)
